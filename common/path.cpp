...
Path& Path::operator=(const Path& path) {
    ...
    __memcpy(str_ptrs, path.str_ptrs, sizeof(char*) * path.count);
    __memcpy(str_tab,  path.str_tab,  path.str_tab_size);
...
void Path::parse(const char* path) {
    ...
    usize len = __strlen(path);
    str_tab = new char[len + 1];
    __memcpy(str_tab, path, len);
    str_tab[len] = 0;
...
Path Path::parent() const {
    ...
    __memcpy(ret.str_ptrs, str_ptrs, sizeof(char*) * ret.count);
    __memcpy(ret.str_tab,  str_tab,  ret.str_tab_size);
