...
u64 CircularBuffer::read(void* out, u64 size) {
    ...
    if (wr_ptr > rd_ptr) {
        __memcpy(out, &buffer[rd_ptr], size);
        rd_ptr += size;
        return size;
    }
    ...
    __memcpy(buf, &buffer[rd_ptr], csize);
    ...
    __memcpy(buf, &buffer[rd_ptr], size - csize);
    rd_ptr += size - csize;
...
u64 CircularBuffer::write(void* in, u64 size) {
    ...
    if (rd_ptr > wr_ptr) {
        __memcpy(&buffer[wr_ptr], in, size);
        wr_ptr += size;
        return size;
    }
    ...
    __memcpy(&buffer[wr_ptr], buf, csize);
    ...
    __memcpy(buf, &buffer[wr_ptr], size - csize);
    wr_ptr += size - csize;
